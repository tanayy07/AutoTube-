...
export default function Home() {
  ...
  const [downloadMode, setDownloadMode] = useState<'direct' | 'telegram'>('telegram')
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <p className="text-xl text-gray-400 mb-12 max-w-2xl mx-auto">
              Download YouTube videos, MP3s, and clips directly from Telegram. 
              No ads, no limits, just pure speed.
            </p>

            {/* URL Input */}
            <div className="max-w-2xl mx-auto mb-8">
              <div className="relative group">
                <div className="absolute -inset-1 bg-gradient-to-r from-red-500 to-blue-500 rounded-2xl blur-lg opacity-25 group-hover:opacity-40 transition duration-1000"></div>
                <div className="relative bg-[#1A1A1A]/80 backdrop-blur-xl border border-white/10 rounded-2xl p-2">
                  <div className="flex items-center">
                    ...
                    />
                    <button 
                      onClick={downloadMode === 'direct' ? handleDirectDownload : () => {
                        if (url) {
                          window.open(`https://t.me/openmind1_bot?start=${encodeURIComponent(url)}`, '_blank');
                        } else {
                          alert('Please enter a YouTube URL first');
                        }
                      }}
                    ...
                    >
                      {isDownloading ? 'Downloading...' : downloadMode === 'direct' ? 'Download Now' : 'Download From Telegram'}
                    </button>
                  </div>
                </div>
              </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <label className="flex items-center space-x-2 cursor-pointer">
                      ...
                      <span className="text-sm text-gray-400">
                        Use cookies for authentication (enables 4K/1440p)
                      </span>
                    </label>
                    ...
                    {showMoreOptions && (
                      <div className="pl-4 border-l-2 border-gray-700">
                        <label className="block text-sm text-gray-400 mb-1">
                          Proxy URL (for regional restrictions)
                        </label>
                        <input
                          type="text"
                          value={proxy}
                        ...
                        <p className="text-xs text-gray-600 mt-1">
                          Only needed if videos are blocked in your region
                        </p>
                      </div>
                    )}
                  </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
              <Link 
                href="https://t.me/openmind1_bot"
                className="group relative inline-flex items-center space-x-2 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white px-8 py-4 rounded-xl font-semibold transition-all transform hover:scale-[1.02]"
              >
              ...
              <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-4">
                <div className="w-32 h-32 bg-white/10 rounded-lg flex items-center justify-center">
                  <span className="text-xs text-gray-500">Bot QR Code</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            {[
              ...
              <div key={index} className="group relative">
                <div className={`absolute -inset-0.5 bg-gradient-to-r ${feature.color} opacity-0 group-hover:opacity-100 blur-lg transition duration-500`}></div>
                <div className="relative bg-[#1A1A1A]/50 backdrop-blur-xl border border-white/10 rounded-2xl p-6 h-full hover:border-white/20 transition-all">
                  <div className={`w-12 h-12 bg-gradient-to-r ${feature.color} rounded-xl flex items-center justify-center mb-4 text-white`}>
                    {feature.icon}
                  </div>
      ...
      <footer className="relative z-10 px-6 lg:px-12 py-12 border-t border-white/5">
        <div className="max-w-7xl mx-auto text-center">
          <p className="text-gray-500 text-sm">
            Built with Next.js, BullMQ, and yt-dlp • Max file size: 50MB
          </p>
        </div>
      </footer>
    </main>
